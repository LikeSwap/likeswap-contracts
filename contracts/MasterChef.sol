...
contract LikeToken is BEP20 {
    ...
    // Min amount to liquify. (default 500 Likes)
    uint256 public minAmountToLiquify = 500 ether;
    // The swap router, modifiable. Will be changed to Like's router when our own AMM release
    IUniswapV2Router02 public LikeRouter;
    // The trading pair
    address public LikePair;
    // In swap and liquify
    ...
    event LikeRouterUpdated(address indexed operator, address indexed router, address indexed pair);
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "Like::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    /**
     * @notice Constructs the Like Token contract.
     */
    constructor() public BEP20("Like Token", "Like") {
    ...
    /// @dev overrides transfer function to meet tokenomics of Like
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        // swap and liquify
        if (
            swapAndLiquifyEnabled == true
            && _inSwapAndLiquify == false
            && address(LikeRouter) != address(0)
            && LikePair != address(0)
            && sender != LikePair
            && sender != owner()
        ) {
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            require(taxAmount == burnAmount + liquidityAmount, "Like::transfer: Burn value invalid");

            // default 95% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "Like::transfer: Tax value invalid");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the Like pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = LikeRouter.WETH();

        _approve(address(this), address(LikeRouter), tokenAmount);

        // make the swap
        LikeRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        LikeRouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
    ...
    // To receive BNB from LikeRouter when swapping
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "Like::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(_burnRate <= 100, "Like::updateBurnRate: Burn rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "Like::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
     ...
     * Can only be called by the current operator.
     */
    function updateLikeRouter(address _router) public onlyOperator {
        LikeRouter = IUniswapV2Router02(_router);
        LikePair = IUniswapV2Factory(LikeRouter.factory()).getPair(address(this), LikeRouter.WETH());
        require(LikePair != address(0), "Like::updateLikeRouter: Invalid pair address.");
        emit LikeRouterUpdated(msg.sender, address(LikeRouter), LikePair);
    }
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "Like::transferOperator: new operator is the zero address");
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function pendingLike(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 pending = user.amount.mul(accLikePerShare).div(1e12).sub(user.rewardDebt);
        return pending.add(user.rewardLockedUp);
    }

    // View function to see if user can harvest Likes.
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        payOrLockupPendingLike(_pid);
        if (_amount > 0) {
            pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
            if (address(pool.lpToken) == address(Like)) {
                uint256 transferTax = _amount.mul(Like.transferTaxRate()).div(10000);
    ...
    // Pay or lockup pending Likes.
    function payOrLockupPendingLike(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                // send rewards
                safeLikeTransfer(msg.sender, totalRewards);
    ...
    function updateEmissionRate(uint256 _LikePerBlock) public onlyOwner {
        massUpdatePools();
        emit EmissionRateUpdated(msg.sender, LikePerBlock, _LikePerBlock);
        LikePerBlock = _LikePerBlock;
    }
